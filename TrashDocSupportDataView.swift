...
/* If docRowHeight is too small, the List itself will sprout its own vertical scroller, resulting
 in the dreaded scroll view inside a scroll view. */
...
class CleanDocSupportFilesController : NSWindowController {
    ...
    @objc func makeWindow() {
        ...
        if (docSupportBag.docSupports.count > 0) {
            ...
            height += 301.0 // For the long explanatory text which is at the top of the window
            height += 47.0 // For the button and its margins which area the bottom of the window
            height += docRowHeight * CGFloat(docSupportBag.docSupports.count) // For the table in the middle of the window
        }
        if (docSupportBag.oldAppVersionCrufts.count == 0 && docSupportBag.docSupports.count == 0) {
            height += 50  //  For the "No document support files found…" text
        }
        ...
        /* From documentation: The main screen refers to the screen containing the window that is currently receiving keyboard events. It is the main screen because it is the one with which the user is most likely interacting."  I would prefer to use NSWindow.screen, but we've not created the window yet.  So I guess it makes sense to use the .main screen */
        ...
        let margin = 32.0
        let screenUseableHeight = mainScreen.visibleFrame.height - margin

        if (height > screenUseableHeight) {
            height = screenUseableHeight
        }
        
        /*
        From dox of NSHostingController.sizingOptions,
        If an NSHostingController has a frame that is smaller or larger than that required to display its SwiftUI content, the content will be centered within that frame.
        */
            ...
            VStack {
                if let oldFilesAdvice = self.oldFilesAdvice {
                    ...
                    Button(action: {self.trashAllOldFiles()}) {
                        Text(NSLocalizedString("Trash checkmarked files now", comment: "Label on a button"))
                    }
                }
                ...
                if let currentFilesAdvice = self.currentFilesAdvice {
                    ...
                    .frame(width: usingWidth, height:10)
                    .padding(EdgeInsets(top: 0.0, leading: (listLeadingInset + 5.0), bottom: 0.0, trailing: 0.0))
                    
                    Divider().padding(EdgeInsets(top: 1.0, leading: 0.0, bottom: 1.0, trailing: 0.0))
                    
                    ScrollView {
                        /* If we were deploying to macOS 12 or later, we could replace the following
                         List with a Table, which looks much nicer, particularly because it features
                         column headers.  Example:
                         https://www.hackingwithswift.com/quick-start/swiftui/how-to-create-multi-column-lists-using-table
                         But when this code was written we still suppprt users with macOS 11 :(  */
                        List {
                                ...
                                HStack {
                                    HStack {
                                        ...
                                        /* The "?" will cause it to be sorted to the top of other names. */
                                            .truncationMode( .tail)
                                            .padding(EdgeInsets(top: 0.0, leading: 0.0, bottom: 0.0, trailing: 0.0))
                                        /* Per dox, the following has no effect if the text is only one line. */
                                    ...
                                    TableBodyCell((docSupportBag.docSupports[docSupport.index!]).exids!, columnWidth)
                                    TableBodyCell((docSupportBag.docSupports[docSupport.index!]).settings!, columnWidth)
                                    TableBodyCell((docSupportBag.docSupports[docSupport.index!]).syncLogs!, columnWidth)
                                }
                            }
                        }
                        /* https://stackoverflow.com/questions/58242342/swiftui-row-height-of-list-how-to-control */
                        .frame(height: CGFloat(self.docSupportBag.docSupports.count)*docRowHeight)
                    }
                    
                    /* List seems to have an annoying margin of about 15 points on
                     the sides and 5 points on the top and bottom.  To demonstrate this,
                     uncomment this line to make a red border: */
                    //.border(.red)
                    /* You can partially get rid of this margin by passing
                     ...
                     to clip a neighboring view. */
                        .padding(EdgeInsets(top: -5.0, leading: listLeadingInset, bottom: -5.0, trailing: -19.0))
                    ...
                    HStack {
                        Button(action: {self.trashIfDoCurrentFiles()}) {
                            Text(NSLocalizedString("Trash data files so checkmarked now", comment: "Label on a button"))
                        }
