...
/*!
 @details  The loop was added to this methodin BkmkMgrs 3.1, when it wqs
 observed that the array returned by the system mothod
 ...
 I understand from the documentation of
 ...
 app if the system does not know the app is running yet.  But that is not the
 problem because BkmxAgent was running for many minutes.  After updating to
 macOS 14.4 Beta 4 and then 5 it got much better, maybe 0.1% failure rate.
 Also, I tried it with TextEdit.  Running it all day, it even failed to find
 the running TextEdit a few times.
 
 Another fix I tried was to add the following  method to SSYOtherApper
 
 + (NSArray<SSYRunningApp*>*)runningAppsWithExecutableName:(NSString*)executableName
 andBundleIdentifier:(NSString*)bundleIdentifier
 error_p:(NSError**)error_p;
 
 This method ran `ps` instead of using NSRunningApplication, found the desired
 executable name, and then dug into the app's Info.piist to get the bundle
 identifier.  Whew!  Also, notice that it returned SSYRunningApp, my stripped-
 down replacement for NSRunningApplication, because there is no API to create
 and set properties in NSRunningApplication.  It was a lot of work.  Result:
 It did not help.  Whenever NSRunningApplication failed, this new method based
 on `ps` failed also.  So I delete that method from SSYOtherApper.
 
 After completing this work, I retested in macOS 14.4 Beta 5 I think I have
 now.  I clickd in the main menu: Show Syncing Status 10 times.  It required
