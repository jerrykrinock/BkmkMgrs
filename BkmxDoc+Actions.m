...
- (IBAction)upgradeInsecureBookmarks:(id)sender {
    ...
    NSInteger countSecure = 0 ;
    NSInteger countInsecure = 0 ;
    for (Stark* stark in [[[self starker] root] extractLeavesOnly]) {
...
#define MINUTES_PER_UPGRADE_BOOKMARK 0.015f
    NSString* upgrade = @"Upgrade" ;
    NSString* msg = [[NSString alloc] initWithFormat:
                     @"Many websites are now secured with SSL certificates.  "
                     @"You should visit these sites using the newer, more secure https:// protocol.\n\n"
                     @"This document contains %tu secure bookmarks and %tu insecure old-style http:// bookmarks.  "
                     @"Clicking '%@' will test each insecure bookmark, and upgrade all which respond to the more secure https:// protocol.  "
                     @"It may take %tu or so minutes to run this.",
                     countSecure,
                     countInsecure,
                     upgrade,
                     (NSUInteger)ceil(countInsecure * MINUTES_PER_UPGRADE_BOOKMARK)] ;
    SSYAlert* alert = [[SSYAlert alloc] init] ;
    [alert setSmallText:msg] ;
    [msg release] ;
    [alert setIconStyle:SSYAlertIconInformational] ;
    [alert setButton1Title:upgrade] ;
    [alert setButton2Title:[[BkmxBasis sharedBasis] labelCancel]] ;
    [alert doooLayout] ;
    ...
    [hostWindow beginSheet:[alert window]
         completionHandler:^void(
             ...
             /* If user clicks "Upgrade", modalResponse = 1000.
              If user clicks "Cancel", modalResponse = 1002 */
             if (modalResponse == NSAlertFirstButtonReturn) {
                 NSMutableArray* insecureStarks = [[NSMutableArray alloc] init] ;
                 for (Stark* stark in [[[self starker] root] extractLeavesOnly]) {
                     NSString* url = [stark url] ;
                     if ([url hasPrefix:@"http://"]) {
                         [insecureStarks addObject:stark] ;
                     }
                 }
                 
                 dispatch_queue_t aSerialQueue = dispatch_queue_create(
                                                                       "com.sheepsystems.BkmkMgrs.upgradeInsecureBookmarks",
                                                                       DISPATCH_QUEUE_SERIAL
                                                                       ) ;

                 dispatch_async(aSerialQueue, ^{
                     [self setAutoSaveInhibited:YES] ;
                     
                     /* Note that -[Broker verifyStarks::::::] is supposedly
                      thread-safe. */
                     [[self broker] verifyStarks:insecureStarks
                                      verifyType:BrokerVerifyTypeSecurifyFirstPass
                                           since:[NSDate date]
                                   plusAllFailed:NO
                                ignoreNoInternet:NO
                                   waitUntilDone:YES
                     ...
                     NSMutableArray* securifyCandidates = [[NSMutableArray alloc] init] ;
                     
                     /* Core Data access ahead - must use main thread. */
                     dispatch_sync(dispatch_get_main_queue(), ^{
                         for (Stark* stark in insecureStarks) {
                             if ([stark verifierCodeValue] == 200) {
                                 [securifyCandidates addObject:stark] ;
                             }
                         }
                     }) ;
                     
                     [[self broker] verifyStarks:securifyCandidates
                                      verifyType:BrokerVerifyTypeSecurifySecondPass
                                           since:[NSDate date]
                                   plusAllFailed:NO
                                ignoreNoInternet:NO
                                   waitUntilDone:YES
                     ...
                     dispatch_sync(dispatch_get_main_queue(), ^{
                         NSString* resultMsg = [[NSString alloc] initWithFormat:
                                                @"Upgrade Insecure Bookmarks result:\n\n"
                                                @"• %td insecure bookmarks were upgraded.\n\n"
                                                @"• %td insecure bookmarks could not be upgraded because their site does not respond to https://.\n",
                                                [[self broker] nSecurifySucceeded],
                                                [[self broker] nSecurifyFailed]] ;
                         SSYAlert* showResultsAlert = [[SSYAlert alloc] init] ;
                         [showResultsAlert setSmallText:resultMsg] ;
                         [resultMsg release] ;
                         [showResultsAlert setIconStyle:SSYAlertIconInformational] ;
                         [showResultsAlert setButton1Title:@"OK"] ;
                         [showResultsAlert doooLayout] ;
                         
                         [hostWindow beginSheet:[showResultsAlert window]
                              completionHandler:^void(
                                                      NSModalResponse modalResponse) {
                                  [showResultsAlert release] ;
                              }
                          ] ;
                         
                         [insecureStarks release] ;
                         [securifyCandidates release] ;
                     }) ;

                     [self setAutoSaveInhibited:NO] ;
