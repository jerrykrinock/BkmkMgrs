...
extern NSString* BkmxLazyViewErrorDomain ;

@protocol BkmxLazyViewWindowController
...
/*
 ...
 @details  The notification object is the window in which the BkmxLazyView object
 resides.  Note that the loading of the payload, and hence the posting of this
 notification, occurs no more than once during the lifetime of an BkmxLazyView
 object.
 */
extern NSString* BkmxLazyViewWillLoadPayloadNotification;

/*
 @brief    Notification which is posted after the payload view loads.
 
 @details  Ditto from BkmxLazyViewWillLoadPayloadNotification.
 */
extern NSString* BkmxLazyViewDidLoadPayloadNotification;

/*
 @brief    A companion to BkmxLazyView; provides a method by which a tab view
 item whose view is an instance of BkmxLazyView can look down and see if their
 view has been payloaded
 */
@interface NSTabViewItem (SSYLazyPayload)

/*
 @brief    If the receiver's view is an instance of BkmxLazyView, returns 
...
/*
 ...
 @details  Do not use this class if you are deploying to macOS 10.10 or later.
 Instead, use the new NSTabViewController and friends.  It does all of the
 lazy loading stuff.
 
 OK, then, for old designs…
 
 When the receiver loads its nib as a result of being moved to a
 ...
 */
@interface BkmxLazyView : NSView {
    BOOL m_isPayloaded ;
    
    // Needed in macOS 10.8 or later…
