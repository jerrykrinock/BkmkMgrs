...
/* Note: This is the base class for Mozilla-engine browsers (Firefox, Zen, etc.)
 * Subclasses MUST override +constants_p to provide browser-specific constants,
 * particularly ownerAppDisplayName and appSupportRelativePath.
 * This base class constants struct is only used if a subclass fails to override. */
static const ExtoreConstants extoreConstants = {
	...
	/* ownerAppDisplayName */             @"Firefox-Based Browser", // Subclasses should override
	/* webHostName */                     nil,
	...
	/* oAuthRealm */                      nil,
	/* appSupportRelativePath */          nil, // Subclasses MUST override
...
/* Subclasses MUST override this method to return their specific bundle identifiers. */
+ (NSArray*)browserBundleIdentifiers {
    return [NSArray array] ; // Empty - subclasses must override
}
...
/* Subclasses should override these profile discovery methods.
 * The base class returns nil/empty to indicate that subclass must provide
 * specific profile discovery logic. */
		...
		if (mozItemType == kMozItemTypeStark) {
			...
			if (mozBookmarkType == kMozBookmarksTypeBookmark) {
				...
				if (mozBookmarkType != kMozBookmarksTypeFolder) {
					NSLog(@"Internal Error 523-5981 -[ExtoreFirey put].  Treating unrecognized moz_bookmarks type %ld as folder.  title=%@.", (long)mozBookmarkType, [stark name]) ;
				}
	...
	if (!bookmarkID) {
        ...
        if (newContent != nil) {
            if (mozContent != nil) {
                ...
                NSArray* columns = [ExtoreFirey mozItemsAnnosColumns] ;
...
- (void)writeUsingStyle1InOperation:(SSYOperation*)operation {
	...
	NSArray* columns = [ExtoreFirey mozBookmarksColumns] ;
	...
	for (stark in starks) {
		...
		if (mozBookmarkItem != nil) {
			...
			if (!isHartainer) {
				...
				if (mozType != [ExtoreFirey mozBookmarksTypeForSharype:docSharype]) {
					matchSharype = NO ;
				}
				...
				if (!matchAllInMozBookmarks) {
					...
					if (!matchSharype) {
						NSInteger type = [ExtoreFirey mozBookmarksTypeForSharype:docSharype] ;
			...
			[values addObject:[NSNumber numberWithInteger:[ExtoreFirey mozBookmarksTypeForSharype:sharype]]] ;
...
- (NSNumber*)insertIntoMozTablesPlaceForRootName:(NSString*)rootName
	...
	NSString* query = [SSYSqliter queryInsertIntoTable:@"moz_bookmarks"
											   columns:[ExtoreFirey mozBookmarksColumns]
												values:values] ;
	[sqliter runQuery:query
				error:&error ] ;
	if (error != nil) {
	...
	if (itemAnnoIDAllBookmarks != nil) {
		...
		query = [SSYSqliter queryInsertIntoTable:@"moz_items_annos"
										 columns:[ExtoreFirey mozItemsAnnosColumns]
										  values:values] ;
	}
	[sqliter runQuery:query
				error:&error ] ;
...
- (NSString*)messagePleaseActivateThisProfile
{
    return [NSString stringWithFormat:
            ...
            @"\n"
            @"\nPlease run %@ in profile '%@', and ensure that no other instances of this browser are running."
            @"\n"
...
- (BOOL)installMessengerError_p:(NSError**)error_p {
    ...
    NSDictionary* moreInfos = @{@"allowed_extensions" : @[
                                        [ExtoreFirey extension1Uuid],
                                        [ExtoreFirey extension2Uuid]
